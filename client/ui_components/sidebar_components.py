...
import traceback
from services.mcp_service import connect_to_mcp_servers, get_prompt_from_client
...
def create_history_chat_container():
    history_container = st.sidebar.container(height=100, border=None)
...
def show_dialog():
    csv_path = st.text_input("CSV Path", value="", placeholder="Enter CSV file path")
    topic = st.text_input("Topic", value="", placeholder="Enter topic for exploration")
    if st.button("Submit"):
        st.session_state.prompt = run_async(get_prompt_from_client(
            st.session_state.client,
            "explore-data",
            {"csv_path": csv_path, "topic": topic}
        ))[0].content
        # st.markdown(f"Prompt created: `{type(st.session_state.prompt[0].content)}`")
        st.rerun()
